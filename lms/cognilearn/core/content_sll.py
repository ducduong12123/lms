...
ITEMS: list[dict[str, Any]] = [
	_item(
		...
		prerequisites=["node_structure", "head_pointer"],
		misconception_codes=["lost_successor"],
	...
	_item(
		...
		misconception_by_answer={
			"A": "overwrite_before_linking",
			"C": "head_confusion",
			"D": "wrong_statement_order",
		},
		hint_ladder=[
			"Một câu lệnh phải nối new_node với successor cũ trước.",
			"Hãy đọc hai câu lệnh theo thứ tự thời gian.",
		],
	),
	_item(
		"SLL-PILOT-B-03",
		...
		hint_ladder=[
			"k = 0 là node đầu tiên, không phải vị trí trước head.",
			"Hãy viết lại successor của node 3 sau phép chèn.",
		],
	),
	_item(
		"SLL-PILOT-B-04",
		...
		correct="C",
		concepts=["sll_insertion_after_k", "edge_cases"],
		prerequisites=["node_structure", "sll_next_pointer"],
		misconception_codes=["tail_boundary_confusion"],
		misconception_by_answer={
			"A": "head_confusion",
			"B": "tail_boundary_confusion",
			"D": "cycle_creation",
		},
		hint_ladder=[
			"Node cuối hiện tại không có successor.",
			"Sau khi chèn sau tail, node mới sẽ trở thành tail.",
		],
	),
	_item(
		"SLL-PILOT-G-01",
	...
	_item(
		...
		difficulty="medium",
		correct="new_node->next = node_k->next; node_k->next = new_node",
		concepts=["sll_insertion_after_k", "sll_next_pointer"],
		prerequisites=["node_structure", "kth_node_access", "sll_next_pointer"],
		options=[("A", "node_k->next = new_node"), ("B", "new_node->next = node_k->next")],
		answer_mode="ordered_tokens",
		expected_tokens=["new_node->next", "node_k->next"],
		accepted_answers=["new_node->next = node_k->next; node_k->next = new_node"],
		misconception_codes=["overwrite_before_linking", "wrong_statement_order"],
		hint_ladder=[
			"Câu lệnh đầu tiên cần dùng successor cũ của node_k.",
			"Sau đó mới cho node_k trỏ tới new_node.",
		],
	),
	_item(
		"SLL-PILOT-G-03",
		...
		concepts=["sll_insertion_after_k", "sll_next_pointer"],
		prerequisites=["node_structure", "sll_next_pointer"],
		misconception_codes=["overwrite_before_linking"],
		misconception_by_answer={
			"A": "overwrite_before_linking",
			"C": "head_confusion",
			"D": "cycle_creation",
	...
	_item(
		...
		hint_ladder=[
			"So sánh successor của new_node trong từng lựa chọn.",
			"new_node không nên trỏ nullptr nếu còn node phía sau.",
		],
	),
	_item(
		"SLL-PILOT-I-01",
		...
		difficulty="medium",
		options=[
			("A", "6->next = 9 và 9->next = nullptr"),
			("B", "2->next = 9 và 9->next = 6"),
			("C", "9->next = 2 và 2->next = 6"),
			("D", "head = 9 và 9->next = 2"),
		],
		correct="A",
		concepts=["sll_insertion_after_k", "kth_node_access"],
		prerequisites=["node_structure", "head_pointer", "traversal_counting"],
	...
	_item(
		...
		difficulty="medium",
		options=[
			("A", "new_node->next = node_k->next"),
			("B", "node_k = new_node"),
			("C", "head = node_k"),
			("D", "node_k->next = nullptr"),
		],
		correct="A",
		concepts=["sll_insertion_after_k", "sll_next_pointer"],
		prerequisites=["node_structure", "sll_next_pointer"],
		...
		misconception_by_answer={
			"B": "wrong_statement_order",
			"C": "head_confusion",
			"D": "tail_boundary_confusion",
		},
	),
	_item(
		"SLL-PILOT-R-02",
